...
public class Expr_LogicalOr(ushort line, ushort col) : Expr_Binary(line, col)
{
    public override void Compile(ILGenerator il, Prototype proto)
    {
        ...
        il.Emit(OpCodes.Brtrue, shortcut); // First operand true, shortcut and don't evaluate second
        il.Emit(OpCodes.Pop);// First was duplicated to return if true, but it wasn't, need to pop
